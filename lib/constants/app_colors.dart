import 'package:flutter/material.dart';

class AppColors {
  static const Color primaryColor = Color(0xFF87CEEB); // Sky Blue
  static const Color secondaryColor = Color(0xFF000000); // Black
  static const Color accentColor = Color(0xFFFF7029); // Orange
  static const Color backgroundColor = Color(0xFFFFFFFF); // White
  static const Color discountColor = Color(0xFF008000); // Dark Blue
  static const Color buttonColor = Color(0xFF031cfc);
  static const Color headingColor = Color.fromARGB(255, 37, 150, 195);
  static const Color chartBackground = Color(0xFFFFFFFF);

  static const Color chartTextColor = Color(0xFF000000);

  static const Color chartGridColor = Color(0xFFCCCCCC);

  static const Color chartLineColor = Color(0xFF4CAF50);
  static const Color chartAreaColor = Color(0xFF00FF00);
}
